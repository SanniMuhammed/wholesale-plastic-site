-- Keep public order creation behind the validated RPC.
-- Direct table inserts are not exposed to anon/authenticated roles.
...
begin
  if p_order is null or jsonb_typeof(p_order) <> 'object' then raise exception 'Invalid order payload'; end if;
  ...
  if exists (select 1 from jsonb_array_elements(p_items) x where jsonb_typeof(x) <> 'object' or coalesce(length(trim(x->>'product_slug')),0)=0 or length(x->>'product_slug') > 160 or coalesce((x->>'quantity')::integer,0) < 1 or coalesce((x->>'quantity')::integer,0) > 1000000) then raise exception 'Invalid order item'; end if;
  select count(*) into v_item_count from (select value->>'product_slug' slug, sum((value->>'quantity')::integer)::bigint quantity from jsonb_array_elements(p_items) group by value->>'product_slug') q where q.quantity <= 1000000;
...
grant execute on function public.create_wholesale_order(jsonb, jsonb) to anon, authenticated;
