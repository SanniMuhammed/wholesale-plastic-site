...
export interface ProductImage { id: string; product_id: string; storage_path: string; sort_order: number; is_main: boolean; }
export interface ProductReview { id: string; product_id: string; customer_name: string; business_name: string | null; location: string | null; rating: number; review_en: string; review_fr: string; customer_photo_path: string | null; is_published: boolean; sort_order: number; created_at: string; updated_at: string; }
export interface Product {
  id: string; slug: string; category_id: string | null; name_en: string; name_fr: string;
  short_description_en: string; short_description_fr: string; description_en: string; description_fr: string;
  capacity: string | null; material_en: string; material_fr: string; packaging_en: string; packaging_fr: string;
  use_case_en: string; use_case_fr: string; wholesale_only: boolean; availability_status: AvailabilityStatus;
  pricing_mode?: PricingMode; price?: number | null; price_unit?: string | null;
  is_featured: boolean; status: ProductStatus; sort_order: number; legacy_slug: string | null;
  created_at: string; updated_at: string; category?: Category | null; images?: ProductImage[]; colors?: Color[]; reviews?: ProductReview[];
}
