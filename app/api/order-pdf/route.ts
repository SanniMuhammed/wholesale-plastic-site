...
function line(cmds: string[], x: number, y: number, text: string, size: number, font = "F1") {
  cmds.push(`BT /${font} ${size} Tf ${x} ${y} Td ${pdfText(text)} Tj ET`);
}

function multiLine(cmds: string[], x: number, y: number, text: string, size: number, maxChars: number, leading = 13) {
  const lines = wrap(text, maxChars);
  lines.forEach((item, index) => line(cmds, x, y - index * leading, item, size));
...
function buildPdf(data: PdfPayload): Uint8Array {
  ...
  const labels = isFr
    ? { order: "COMMANDE EN GROS", ref: "Référence", date: "Date", customer: "Client", business: "Entreprise", destination: "Destination", delivery: "Livraison", item: "Produit", capacity: "Format", qty: "Qté", notes: "Notes", footer: "Prix et disponibilité à confirmer avec notre équipe via WhatsApp.", thank: "Merci de votre intérêt pour Sherinab Venture." }
    : { order: "WHOLESALE ORDER", ref: "Reference", date: "Date", customer: "Customer", business: "Business", destination: "Destination", delivery: "Delivery", item: "Product", capacity: "Format", qty: "Qty", notes: "Notes", footer: "Pricing and availability are confirmed by our team via WhatsApp.", thank: "Thank you for your interest in Sherinab Venture." };
  ...
  line(cmds, 390, 779, `${labels.ref}: ${data.reference}`, 8);
  line(cmds, 390, 764, `${labels.date}: ${date}`, 8);
  ...
  if (data.businessName) multiLine(cmds, leftX + 12, leftY - 3, data.businessName, 8, 34, 11);
  line(cmds, rightX + 12, y - 20, labels.destination.toUpperCase(), 7, "F2");
  const destination = [data.city, data.country].filter(Boolean).join(", ");
  multiLine(cmds, rightX + 12, y - 39, destination || "—", 10, 30);
  if (data.deliveryPreference) multiLine(cmds, rightX + 12, y - 68, `${labels.delivery}: ${data.deliveryPreference}`, 7.5, 38, 10);
  ...
  for (const item of data.items) {
    const nameLines = wrap(item.name, 42);
    const rowH = Math.max(26, nameLines.length * 11 + 10);
    if (y - rowH < 120) break;
    nameLines.forEach((name, index) => line(cmds, MARGIN + 8, y - index * 11, name, 9));
  ...
  if (data.note) {
    y -= 10;
    line(cmds, MARGIN, y, labels.notes.toUpperCase(), 7, "F2");
  ...
  cmds.push(`${MUTED} rg`);
  line(cmds, MARGIN, 44, "sherinabventure.com", 7);
