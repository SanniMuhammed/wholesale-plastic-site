...
export default async function ContactPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const countriesServed = settings?.countries_served ?? [];
    ...
    <div className="mx-auto max-w-content px-4 py-14 sm:px-6 sm:py-16">
      ...
      <div className="mt-10 grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.7fr)]">
        ...
        <aside className="rounded-2xl border border-border bg-brand-light/40 p-5 sm:p-6">
          ...
          {countriesServed.length > 0 && <div className="mt-5 flex items-start gap-3"><Globe2 size={17} className="mt-0.5 shrink-0 text-brand" /><div><p className="text-xs text-muted">{locale === "fr" ? "Pays desservis" : "Countries served"}</p><p className="mt-1 text-sm leading-relaxed text-ink">{countriesServed.join(" · ")}</p></div></div>}
