...
export default async function ProductDetailPage({
  ...
  return (
    <div className="mx-auto max-w-content px-4 py-8 sm:px-6 sm:py-12">
      <Link href={`${base}/products`} className="inline-flex text-sm text-muted transition-colors hover:text-ink">
        ...
        <div className="lg:sticky lg:top-24">
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-xs text-muted">
            ...
            {product.wholesaleOnly && (
              <span className="rounded-full border border-brand/30 bg-brand-light px-3 py-1.5 font-medium text-brand">
        ...
        <div>
          ...
          <div className="mt-8">
            <div className="flex items-end justify-between gap-4">
              <h2 className="font-display text-xl font-semibold text-ink">{dict.common.productDetails}</h2>
              <span className="font-mono text-[10px] uppercase tracking-wider text-muted">Wholesale spec</span>
            </div>
            <dl className="mt-3 overflow-hidden rounded-xl border border-border">
              {product.capacity && (
                <div className="rule-row px-4">
              ...
              <div className="rule-row px-4">
                ...
                <dd className="max-w-[60%] text-right font-mono text-sm font-medium text-ink">{product.material[locale]}</dd>
              </div>
              <div className="rule-row px-4">
                <dt className="text-sm text-muted">{dict.common.packaging}</dt>
                <dd className="max-w-[60%] text-right font-mono text-sm font-medium text-ink">{product.packaging[locale]}</dd>
              </div>
              <div className="rule-row px-4">
                <dt className="text-sm text-muted">{dict.common.useCase}</dt>
                <dd className="max-w-[60%] text-right font-mono text-sm font-medium text-ink">{product.useCase[locale]}</dd>
              </div>
            </dl>
          </div>
