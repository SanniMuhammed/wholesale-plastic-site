      ...
      <div className="mx-auto max-w-content px-4 py-16 sm:px-6 sm:py-20">
        ...
        <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {BUSINESS_TYPES.map((item, index) => {
            ...
            return (
              <Link key={text.title} href={`/${locale}/products?category=${item.category}`} className="group rounded-lg border border-border bg-surface p-5 transition-all hover:-translate-y-0.5 hover:border-brand/40 hover:shadow-lifted">
                <div className="flex items-start justify-between gap-4"><span className="font-mono text-xs font-bold text-brand/60">{String(index + 1).padStart(2, "0")}</span><span className="text-lg text-ink-soft transition-transform group-hover:translate-x-1" aria-hidden>→</span></div>
        ...
        <div className="mt-12 border-t border-border pt-9 sm:mt-14 sm:pt-10">
          <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between sm:gap-6">
            <div><p className="eyebrow text-muted">{copy.next}</p><h3 className="mt-1 font-display text-xl font-semibold text-ink sm:text-2xl">{copy.nextTitle}</h3></div>
            <Link href={`/${locale}/order-summary`} className="text-sm font-semibold text-brand underline decoration-brand/30 underline-offset-4 hover:decoration-brand">{dict.startBusiness.cta} →</Link>
          </div>
          <div className="mt-6 grid gap-2 sm:grid-cols-3 sm:gap-3">
            {PATHS.map(({ key, tint, ink }, i) => {
              const segment = dict.startBusiness.segments[key];
              return (
                <div key={key} className={cx("flex items-center gap-4 px-4 py-4 sm:px-5", tint)}>
                  <span className={cx("font-mono text-xs font-bold", ink)}>{String(i + 1).padStart(2, "0")}</span>
                  <PathIllustration path={key} className={cx("h-8 w-8 shrink-0", ink)} />
                  <div className="min-w-0"><h4 className="font-display text-base font-semibold text-ink">{segment.title}</h4><p className="mt-0.5 text-xs leading-5 text-ink-soft sm:text-sm">{segment.description}</p></div>
                </div>
              );
            })}
          </div>
