  ...
  if (quantity > 0) {
    return (
      <div className={cx("flex h-11 w-full items-center overflow-hidden rounded border border-brand bg-brand text-surface", className)} aria-label={`${productName}, ${quantity} in order`}>
        <button type="button" onClick={() => removeItem(slug)} aria-label={`Remove ${productName} from order`} title={dict.common.removeFromOrder} className="inline-flex h-full w-11 shrink-0 items-center justify-center transition-colors hover:bg-brand-dark active:scale-95">
