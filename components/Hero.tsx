  ...
  return (
    <section className={`relative mx-auto max-w-content overflow-hidden px-4 sm:px-6 ${hasHeroImage ? "min-h-[590px] py-8 sm:min-h-[600px] sm:py-10 md:min-h-[610px] md:py-12 lg:min-h-[620px] lg:py-14" : "pb-16 pt-12 sm:pb-24 sm:pt-16 lg:pt-20"}`}>
      ...
      <div className={`relative z-10 flex items-center ${hasHeroImage ? "min-h-[510px] sm:min-h-[520px] md:min-h-[530px] lg:min-h-[540px]" : ""}`}>
        <div className={`${hasHeroImage ? "w-full max-w-[36rem] sm:w-[76%] sm:max-w-[38rem] md:w-[68%] lg:w-[64%] xl:w-[62%]" : "w-full md:max-w-2xl"}`}>
          <div className="animate-settle-up">
            <p className="eyebrow inline-flex max-w-full -rotate-1 items-center rounded-md border border-brand/20 bg-brand-light/92 px-3 py-1.5 text-brand shadow-sm backdrop-blur-md">{dict.hero.supporting}</p>
            ...
            <p className="mt-5 max-w-[34rem] text-base leading-7 text-ink/75 sm:mt-6 sm:text-lg sm:leading-7">{dict.hero.subtitle}</p>
            <div className="mt-7 flex flex-wrap items-center gap-3 sm:mt-8">
              ...
              <Link href={`${base}/order-summary`} className="inline-flex min-h-11 items-center justify-center rounded-md border border-ink/70 bg-surface/90 px-6 py-3 text-sm font-medium text-ink backdrop-blur-md transition-all duration-200 hover:-translate-y-0.5 hover:bg-surface hover:shadow-card focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink focus-visible:ring-offset-2">{dict.hero.secondaryCta}</Link>
            </div>
            <p className="mt-4 text-xs font-medium tracking-[0.08em] text-ink/65 sm:text-sm">{locale === "fr" ? "Quantités en gros • Livraison nationale • Commandes internationales" : "Wholesale quantities • Nationwide delivery • International orders"}</p>
          </div>
        </div>
      </div>
    </section>
