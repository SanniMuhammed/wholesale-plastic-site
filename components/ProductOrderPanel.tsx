  ...
  return (
    <div className="mt-7 flex flex-col gap-4">
      <div className="rounded-xl border border-border bg-surface p-4 sm:p-5">
        ...
        <div className="mt-5 grid gap-2 sm:grid-cols-2"><button ref={buttonRef} type="button" onClick={handleAdd} className={cx("inline-flex min-h-11 items-center justify-center gap-1.5 rounded bg-accent px-4 py-3 text-sm font-semibold text-brand transition-all hover:bg-accent/90 active:scale-[0.98]", added && "bg-accent/80")}>{added && <Check size={15} strokeWidth={2.5} />}{added ? dict.common.addedToOrder : dict.common.addToOrder}</button><a href={whatsappHref} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-11 items-center justify-center rounded border border-brand px-4 py-3 text-sm font-medium text-brand transition-colors hover:bg-brand hover:text-surface">{hasConfirmedPrice ? (locale === "fr" ? "Confirmer le prix" : "Confirm pricing") : dict.common.requestPrice}</a></div>
        <Link href={`/${locale}/order-summary`} className="mt-3 inline-flex w-full items-center justify-center rounded border border-accent bg-accent px-4 py-2.5 text-xs font-semibold text-brand transition-colors hover:bg-accent/90 sm:border-brand sm:bg-brand sm:text-surface sm:hover:bg-brand-dark">{dict.common.reviewOrder} →</Link>
      </div>
      {!mainActionsVisible && <div className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-surface/95 p-3 backdrop-blur print:hidden sm:hidden"><div className="mx-auto flex max-w-content items-center gap-3"><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-ink">{product.name[locale]}</p><p className="font-mono text-[10px] text-muted">× {quantity.toLocaleString()}</p></div><button type="button" onClick={handleAdd} className={cx("inline-flex shrink-0 items-center justify-center gap-1.5 rounded bg-accent px-4 py-2.5 text-sm font-semibold text-brand transition-all active:scale-95", added && "bg-accent/80")}>{added && <Check size={14} strokeWidth={2.5} />}{added ? dict.common.addedToOrder : dict.common.addToOrder}</button></div></div>}
    </div>
  );
}
