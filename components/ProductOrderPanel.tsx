...
export function ProductOrderPanel({ product, locale, dict, hasConfirmedPrice = false }: { product: Product; locale: Locale; dict: Dictionary; hasConfirmedPrice?: boolean }) {
  ...
  return (
    <div className="mt-7 flex flex-col gap-4">
      ...
      {!mainActionsVisible && <div className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-surface/95 p-3 backdrop-blur print:hidden sm:hidden"><div className="mx-auto flex max-w-content items-center gap-3"><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-ink">{product.name[locale]}</p><p className="font-mono text-[10px] text-muted">× {quantity.toLocaleString()}</p></div><button type="button" onClick={handleAdd} style={added ? { border: "1px solid #FF9900" } : undefined} className={cx("inline-flex shrink-0 items-center justify-center gap-1.5 rounded px-4 py-2.5 text-sm font-semibold transition-all active:scale-95", added ? "bg-surface text-brand hover:bg-surface" : "bg-accent text-brand hover:bg-accent/90")}>{added && <Check size={14} strokeWidth={2.5} />}{added ? dict.common.addedToOrder : dict.common.addToOrder}</button></div></div>}
    </div>
  );
}
