    ...
    <div className="mt-8 flex flex-col gap-5">
      <div className="rounded-xl border border-border bg-surface p-4 sm:p-5">
        <div className="flex items-start justify-between gap-4">
          <div><p className="eyebrow text-brand">{dict.common.wholesalePricing}</p><h2 className="mt-1 font-display text-lg font-semibold text-ink">{dict.common.requestPrice}</h2></div>
        ...
        <p className="mt-2 text-sm leading-relaxed text-muted">{locale === "fr" ? "Le prix dépend de la quantité et de la destination. Demandez un devis pour obtenir le prix exact." : "Your wholesale price depends on quantity and destination. Request a quote for the exact price."}</p>
        ...
        <div className="mt-5 grid gap-3">
          ...
          {region === "nigeria" ? <div className="grid gap-3 sm:grid-cols-2"><label className="text-xs font-medium text-ink-soft">{locale === "fr" ? "État" : "State"}<select value={state} onChange={(e) => setState(e.target.value)} className="mt-1.5 h-10 w-full rounded border border-border bg-surface px-3 text-sm text-ink outline-none focus:border-brand"><option value="">{locale === "fr" ? "Choisir un État" : "Select a state"}</option>{NIGERIAN_STATES.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label className="text-xs font-medium text-ink-soft">{locale === "fr" ? "Ville / zone" : "City / area"}<input value={city} onChange={(e) => setCity(e.target.value)} placeholder={locale === "fr" ? "Ex. Ikeja" : "e.g. Ikeja"} className="mt-1.5 h-10 w-full rounded border border-border bg-surface px-3 text-sm text-ink outline-none placeholder:text-muted focus:border-brand" /></label></div> : region === "international" ? <div className="grid gap-3 sm:grid-cols-2"><label className="text-xs font-medium text-ink-soft">{locale === "fr" ? "Pays" : "Country"}<input value={country} onChange={(e) => setCountry(e.target.value)} placeholder={locale === "fr" ? "Ex. Togo" : "e.g. Togo"} className="mt-1.5 h-10 w-full rounded border border-border bg-surface px-3 text-sm text-ink outline-none placeholder:text-muted focus:border-brand" /></label><label className="text-xs font-medium text-ink-soft">{locale === "fr" ? "Ville" : "City"}<input value={city} onChange={(e) => setCity(e.target.value)} placeholder={locale === "fr" ? "Ex. Lomé" : "e.g. Lome"} className="mt-1.5 h-10 w-full rounded border border-border bg-surface px-3 text-sm text-ink outline-none placeholder:text-muted focus:border-brand" /></label></div> : null}
        </div>

        <div ref={sentinelRef} aria-hidden className="h-px w-full" />
        <div className="mt-5 grid gap-2 sm:grid-cols-2"><button ref={buttonRef} type="button" onClick={handleAdd} className={cx("inline-flex min-h-11 items-center justify-center gap-1.5 rounded bg-brand px-4 py-3 text-sm font-medium text-surface transition-all hover:bg-brand-dark active:scale-[0.98]", added && "bg-brand-dark")}>{added && <Check size={15} strokeWidth={2.5} />}{added ? dict.common.addedToOrder : dict.common.addToOrder}</button><a href={whatsappHref} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-11 items-center justify-center rounded border border-ink px-4 py-3 text-sm font-medium text-ink transition-colors hover:bg-ink hover:text-surface">{dict.common.requestPrice}</a></div>
        <Link href={`/${locale}/order-summary`} className="mt-3 inline-flex w-full items-center justify-center py-1 text-xs font-medium text-muted transition-colors hover:text-ink">{dict.common.reviewOrder} →</Link>
      </div>
      {!mainActionsVisible && <div className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-surface/95 p-3 backdrop-blur print:hidden sm:hidden"><div className="mx-auto flex max-w-content items-center gap-3"><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-ink">{product.name[locale]}</p><p className="font-mono text-[10px] text-muted">× {quantity.toLocaleString()}</p></div><button type="button" onClick={handleAdd} className={cx("inline-flex shrink-0 items-center justify-center gap-1.5 rounded bg-brand px-4 py-2.5 text-sm font-medium text-surface transition-all active:scale-95", added && "bg-brand-dark")}>{added && <Check size={14} strokeWidth={2.5} />}{added ? dict.common.addedToOrder : dict.common.addToOrder}</button></div></div>}
    </div>
  );
}
