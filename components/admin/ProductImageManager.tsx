    ...
    <div className="rounded-lg border border-border bg-surface p-4 sm:p-5">
      <div className="mb-4 flex items-center justify-between">
        ...
        <button
          ...
          disabled={uploading}
          className="inline-flex items-center gap-1.5 rounded border border-border px-3 py-2 text-sm font-medium text-ink-soft hover:border-ink disabled:opacity-60"
        >
      ...
      {images.length === 0 ? (
        <p className="rounded border border-dashed border-border p-6 text-center text-sm text-muted">
          No photos yet. Upload at least one before publishing.
        </p>
      ) : (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {images.map((image, index) => (
            <div key={image.id} className="group relative overflow-hidden rounded border border-border">
              ...
              <img
                src={buildProductImageUrl(image.storage_path)}
                alt=""
                className="aspect-square w-full object-cover"
              />
              {image.is_main && (
                <span className="absolute left-1.5 top-1.5 inline-flex items-center gap-1 rounded-full bg-brand px-2 py-0.5 text-[11px] font-medium text-surface">
              ...
              <div className="flex items-center justify-between gap-1 bg-surface px-1.5 py-1.5">
                <button
                  ...
                  className="rounded p-1.5 text-ink-soft hover:bg-background disabled:opacity-30"
                >
                  <ChevronLeft size={16} />
                </button>
                {!image.is_main && (
                  <button
                    type="button"
                    onClick={async () => {
                      await setMainProductImageAction(productId, image.id);
                      router.refresh();
                    }}
                    className="rounded p-1.5 text-ink-soft hover:bg-background"
