              ...
              return (
                <li key={step.number} className="relative flex gap-4 pb-9 last:pb-0">
                  <div className="relative flex w-2 shrink-0 flex-col items-center">
                    <span className={cx("relative z-10 mt-1.5 shrink-0 rounded-full", isHinge ? "h-2.5 w-2.5 bg-brand" : "h-2 w-2 border border-border bg-surface")} />
                    {!isLast && <><span className="absolute bottom-0 left-1/2 top-4 w-px -translate-x-1/2 bg-border" aria-hidden /><span className="absolute bottom-0 left-1/2 top-4 w-px origin-top -translate-x-1/2 bg-brand" style={{ transform: `translateX(-50%) scaleY(${inView ? 1 : 0})`, transitionProperty: "transform", transitionDuration: "0.6s", transitionTimingFunction: "cubic-bezier(0.4,0,0.2,1)", transitionDelay: `${i * 120}ms` }} aria-hidden /></>}
                  </div>
                  <div className="flex-1"><p className={cx("font-mono text-[11px] font-bold tracking-[0.08em]", isHinge ? "text-brand" : "text-muted")}>{step.number}</p><h3 className={cx("mt-1 font-display font-semibold leading-tight text-ink", isHinge ? "text-[1.7rem]" : "text-2xl")}>{step.title}</h3><p className="mt-1.5 max-w-[30ch] text-sm text-muted">{step.description}</p></div>
                </li>
              );
            })}
          </ol>
            ...
            {steps.map((step, i) => {
              ...
              return (
                <li key={step.number} className="relative flex flex-col">
                  {!isLast && <><span className="absolute left-[6px] top-[6px] h-px w-full bg-border" aria-hidden /><span className="absolute left-[6px] top-[6px] h-px w-full origin-left bg-brand" style={{ transform: `scaleX(${inView ? 1 : 0})`, transitionProperty: "transform", transitionDuration: "0.6s", transitionTimingFunction: "cubic-bezier(0.4,0,0.2,1)", transitionDelay: `${i * 120}ms` }} aria-hidden /></>}
                  <span className={cx("relative z-10 h-3 w-3 shrink-0 rounded-full", isHinge ? "bg-brand" : "border-2 border-brand bg-surface")} />
                  <p className="mt-3 font-mono text-xs font-bold text-muted">{step.number}</p>
                  <h3 className={cx("mt-2 font-display font-semibold leading-[1.05] text-ink", isHinge ? "text-4xl" : isBookend ? "text-3xl" : "text-2xl")}>{step.title}</h3>
