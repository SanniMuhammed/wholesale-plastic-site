...
function sectionText(
  ...
  if (locale === "fr") {
    return {
      title: section?.title_fr || fallbackTitle,
      body: section?.body_fr || fallbackBody,
    };
  }

  return {
    title: section?.title_en || fallbackTitle,
    body: section?.body_en || fallbackBody,
  };
}
...
export function HowItWorksSection({
  ...
  const copy = sectionText(
    section,
    locale,
    dict.howItWorksSection.title,
    dict.howItWorksSection.subtitle,
  );
  const deliveryCopy = sectionText(
    deliverySection,
    locale,
    dict.deliveryTeaser.title,
    dict.deliveryTeaser.subtitle,
  );
              ...
              return (
                <li key={step.number} className="relative flex flex-col">
                  {!isLast && (
                    <>
                      <span
                        className="absolute left-[6px] top-[6px] h-px w-full bg-border"
                        aria-hidden
                      />
                      <span
