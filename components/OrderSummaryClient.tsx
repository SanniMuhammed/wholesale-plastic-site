...
export function OrderSummaryClient({ locale, dict, products }: { locale: Locale; dict: Dictionary; products: Product[] }) {
  ...
  async function handleFallbackSubmit(e: FormEvent<HTMLFormElement>) { e.preventDefault(); setStatus("sending"); try { const res = await fetch("/api/quote", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...details, ...fallback, items: resolvedItems, locale }) }); if (!res.ok) throw new Error("Request failed"); setStatus("success"); clear(); setDetails(EMPTY_DETAILS); setFallback(EMPTY_FALLBACK); } catch { setStatus("error"); } }

  const whatsappHref = orderInquiryLink(dict, { items: resolvedItems.map((i) => ({ name: i.name, quantity: i.quantity })), customerName: details.customerName, businessName: details.businessName, country: details.country, city: details.city, deliveryPreference: details.deliveryPreference, note: details.otherInfo });
  ...
  return <div className="grid gap-10 lg:grid-cols-[1fr,0.9fr]">
    ...
    <div className="print:hidden"><div className="grid gap-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2"><Field label={isFrench ? "Destination country" : "Destination country"} value={details.country} onChange={(v) => updateDetail("country", v)} /><Field label={isFrench ? "Destination city" : "Destination city"} value={details.city} onChange={(v) => updateDetail("city", v)} /></div>
...
function Field({ label, value, onChange, required, type = "text" }: { label: string; value: string; onChange: (v: string) => void; required?: boolean; type?: string }) { return <label className="block"><span className="mb-1 block text-sm font-medium text-ink-soft">{label}</span><input type={type} required={required} value={value} onChange={(e) => onChange(e.target.value)} className="w-full rounded border border-border px-3 py-2.5 text-sm text-ink focus:border-ink focus:outline-none" /></label>; }
function TextAreaField({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) { return <label className="block"><span className="mb-1 block text-sm font-medium text-ink-soft">{label}</span><textarea rows={3} value={value} onChange={(e) => onChange(e.target.value)} className="w-full rounded border border-border px-3 py-2.5 text-sm text-ink focus:border-ink focus:outline-none" /></label>; }
