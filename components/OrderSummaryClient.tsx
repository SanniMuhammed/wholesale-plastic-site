  ...
  return (
    <div className="pb-8">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4 border-y border-border py-4 print:hidden">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">
            {isFrench ? "Vérifiez votre sélection et préparez votre demande." : "Review your selection and prepare your wholesale request."}
          </p>
        </div>
        <span className="font-mono text-xs text-muted">{totalQuantity.toLocaleString()} {dict.common.items}</span>
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1.08fr)_minmax(320px,0.72fr)] lg:items-start">
        <section className="overflow-hidden rounded-2xl border border-border bg-surface shadow-card print:rounded-none print:border-0 print:shadow-none">
          <div className="border-b border-border bg-brand px-5 py-5 text-surface sm:px-7">
            <div className="flex items-start justify-between gap-4">
              ...
              <span className="font-mono text-[10px] text-surface/70">{isFrench ? "COMMANDE" : "ORDER"}</span>
            </div>
            ...
            {hasItems ? (
                ...
                {resolvedItems.map((item) => (
                  ...
                  >
                    <div className="overflow-hidden">
                      <div
                        className={cx(
                          "grid grid-cols-[1fr_auto] gap-4 py-4 transition-opacity",
              ...
              <div className="flex flex-col items-center rounded-xl border border-dashed border-border px-6 py-12 text-center">
                ...
                <p className="mt-1 max-w-sm text-sm leading-6 text-muted">
                  {isFrench ? "Ajoutez les produits dont votre entreprise a besoin pour commencer." : "Add the products your business needs to start building the order."}
                </p>
                <Link href={`${base}/products`} className="mt-5 text-sm font-semibold text-brand hover:underline print:hidden">
                  {sp.browseProducts} →
                </Link>
              </div>
            )}
          ...
          <div className="hidden border-t border-border px-7 py-5 print:block">
            <div className="grid grid-cols-2 gap-6 text-sm">
              ...
              <div><p className="eyebrow text-muted">Destination</p><p className="mt-1 text-ink">{[destinationCity, destinationCountry].filter(Boolean).join(", ") || "—"}</p></div>
            ...
            {details.otherInfo && <div className="mt-6"><p className="eyebrow text-muted">Notes</p><p className="mt-1 text-sm text-ink">{details.otherInfo}</p></div>}
        ...
        <aside className="space-y-6 print:hidden">
          <section className="rounded-2xl border border-border bg-surface p-5 shadow-card sm:p-6">
            ...
            <div className="mt-5 grid gap-4">
              ...
              <label className="block">
                <span className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-muted">{isFrench ? "Livraison" : "Delivery to"}</span>
                <select
              ...
              <label className="block">
                <span className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-muted">{isFrench ? "Préférence de livraison" : "Delivery preference"}</span>
          ...
          <section className="rounded-2xl border border-brand/20 bg-brand-light/45 p-5 sm:p-6">
            ...
            {hasItems ? (
              <a
                ...
                className="mt-5 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-brand px-5 text-sm font-semibold text-surface shadow-sm transition-all hover:bg-brand-dark active:scale-[0.99]"
              >
                <Send size={17} />
                {sp.continueOnWhatsapp}
              </a>
            ) : (
              <span aria-disabled="true" className="mt-5 inline-flex min-h-12 w-full cursor-not-allowed items-center justify-center gap-2 rounded-xl bg-brand px-5 text-sm font-semibold text-surface opacity-50">
                <Send size={17} />
                {sp.continueOnWhatsapp}
              </span>
            )}
          ...
          <div className="border-t border-border pt-5">
            {!showFallback ? (
              <p className="text-sm text-muted">{sp.noWhatsappPrompt} <button type="button" onClick={() => setShowFallback(true)} className="font-semibold text-brand hover:underline">{sp.noWhatsappToggle}</button></p>
            ) : (
              <form onSubmit={handleFallbackSubmit} className="grid gap-4">
                <p className="text-sm text-muted">{sp.fallbackIntro}</p>
                <label className="block">
                  <span className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-muted">{isFrench ? "Livraison" : "Delivery to"}</span>
                  <select required value={fallback.region} onChange={(e) => handleFallbackRegionChange(e.target.value as DeliveryRegion | "")} className="h-11 w-full rounded-lg border border-border bg-surface px-3 text-sm text-ink outline-none focus:border-brand">
                    <option value="">{isFrench ? "Choisir une destination" : "Choose a destination"}</option>
                    <option value="nigeria">{isFrench ? "Nigeria — partout dans le pays" : "Nigeria — anywhere in the country"}</option>
                    <option value="international">{isFrench ? "Hors du Nigeria" : "Outside Nigeria"}</option>
                  </select>
                ...
                {fallback.region === "nigeria" ? (
                  <div className="grid gap-4 sm:grid-cols-2">
                    <SelectField label={isFrench ? "État" : "State"} value={fallback.state} onChange={(v) => updateFallback("state", v)} options={NIGERIAN_STATES} placeholder={isFrench ? "Choisir un État" : "Select a state"} required />
                    <Field label={isFrench ? "Ville / zone" : "City / area"} required value={fallback.city} onChange={(v) => updateFallback("city", v)} />
                  </div>
                ) : fallback.region === "international" ? (
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field label={sp.fallbackFields.country} required value={fallback.country} onChange={(v) => updateFallback("country", v)} />
                    <Field label={sp.fallbackFields.city} required value={fallback.city} onChange={(v) => updateFallback("city", v)} />
                  </div>
                ) : null}
                <Field label={sp.fallbackFields.contact} required value={fallback.contact} onChange={(v) => updateFallback("contact", v)} />
                {status === "error" && <p className="text-sm text-red-700">{sp.errorMessage}</p>}
                <button type="submit" disabled={status === "sending" || !hasItems} className="inline-flex min-h-11 items-center justify-center rounded-xl bg-brand px-5 text-sm font-semibold text-surface disabled:opacity-50">
      ...
      <div className="mt-8 flex flex-wrap items-center justify-between gap-4 border-t border-border pt-5 print:hidden">
        ...
        <button type="button" onClick={handlePrint} disabled={!hasItems} className="inline-flex items-center gap-2 text-sm font-semibold text-ink hover:text-brand disabled:cursor-not-allowed disabled:opacity-40">
          <FileDown size={15} />
          {sp.printReceipt}
        </button>
      </div>
