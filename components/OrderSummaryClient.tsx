...
interface DetailsState { customerName: string; businessName: string; country: string; city: string; deliveryPreference: string; otherInfo: string; }
const EMPTY_DETAILS: DetailsState = { customerName: "", businessName: "", country: "", city: "", deliveryPreference: "", otherInfo: "" };
interface FallbackState { country: string; city: string; contact: string; }
...
export function OrderSummaryClient({ locale, dict, products }: { locale: Locale; dict: Dictionary; products: Product[] }) {
  ...
  function handleRemove(slug: string) {
    ...
    window.setTimeout(() => {
      removeItem(slug);
      setRemovingSlugs((prev) => { const next = new Set(prev); next.delete(slug); return next; });
    }, REMOVE_ANIMATION_MS);
  }
  ...
  async function handleFallbackSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/quote", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...details, ...fallback, items: resolvedItems, locale }) });
  ...
  function handleWhatsAppClick() {
    recordOrderOnWhatsAppClick({ channel: "whatsapp", customerName: details.customerName, businessName: details.businessName, note: details.otherInfo, locale, items: resolvedItems.map((i) => ({ slug: i.slug, name: i.name, capacity: i.capacity, quantity: i.quantity })) });
  }
      ...
      <div className="print:hidden">
        <div className="grid gap-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label={isFrench ? "Pays de destination" : "Destination country"} value={details.country} onChange={(v) => updateDetail("country", v)} />
            <Field label={isFrench ? "Ville de destination" : "Destination city"} value={details.city} onChange={(v) => updateDetail("city", v)} />
          </div>
          ...
          <div className="rounded border border-border bg-brand-light/50 p-4 text-sm text-ink-soft">
            <p className="font-medium text-ink">{isFrench ? "Votre demande sera préparée pour WhatsApp" : "Your request will be prepared for WhatsApp"}</p>
            <p className="mt-1">{isFrench ? "Produits, quantités, destination et préférence de livraison seront inclus dans le message." : "Products, quantities, destination and delivery preference will be included in the message."}</p>
          </div>

          <div className="flex flex-wrap gap-3 pt-2">
            {hasItems ? <a href={whatsappHref} target="_blank" rel="noopener noreferrer" onClick={handleWhatsAppClick} className="inline-flex items-center justify-center rounded bg-brand px-6 py-3 text-sm font-medium text-surface transition-all hover:bg-brand-dark active:scale-[0.98]">{sp.continueOnWhatsapp}</a> : <span aria-disabled="true" className="inline-flex cursor-not-allowed items-center justify-center rounded bg-brand px-6 py-3 text-sm font-medium text-surface opacity-50">{sp.continueOnWhatsapp}</span>}
            <button type="button" onClick={handlePrint} disabled={!hasItems} className="inline-flex items-center justify-center rounded border border-ink px-6 py-3 text-sm font-medium text-ink transition-all hover:bg-ink hover:text-surface active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50">{sp.printReceipt}</button>
          </div>

          <div className="mt-4 border-t border-border pt-4">
            {!showFallback ? <p className="text-sm text-muted">{sp.noWhatsappPrompt}{" "}<button type="button" onClick={() => setShowFallback(true)} className="font-medium text-brand hover:underline">{sp.noWhatsappToggle}</button></p> : <form onSubmit={handleFallbackSubmit} className="grid gap-4"><p className="text-sm text-muted">{sp.fallbackIntro}</p><div className="grid gap-4 sm:grid-cols-2"><Field label={sp.fallbackFields.country} required value={fallback.country} onChange={(v) => updateFallback("country", v)} /><Field label={sp.fallbackFields.city} required value={fallback.city} onChange={(v) => updateFallback("city", v)} /></div><Field label={sp.fallbackFields.contact} required value={fallback.contact} onChange={(v) => updateFallback("contact", v)} />{status === "error" && <p className="text-sm text-red-700">{sp.errorMessage}</p>}<button type="submit" disabled={status === "sending" || !hasItems} className="inline-flex items-center justify-center rounded bg-brand px-6 py-3 text-sm font-medium text-surface transition-all hover:bg-brand-dark active:scale-[0.98] disabled:opacity-60">{status === "sending" ? dict.common.sending : sp.submit}</button></form>}
          </div>
        </div>
      </div>
    </div>
