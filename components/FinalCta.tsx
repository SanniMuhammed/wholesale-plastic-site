      ...
      <div className="mx-auto grid max-w-content grid-cols-2">
        <div className="flex min-w-0 flex-col justify-center px-4 py-12 text-left sm:px-6 lg:px-10 lg:py-20">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-2 sm:gap-3">
            <Link href={`${base}/order-summary`} className="inline-flex items-center justify-center rounded bg-surface px-4 py-2.5 text-xs font-medium text-brand-dark transition-opacity hover:opacity-90 sm:px-6 sm:py-3 sm:text-sm">{dict.finalCta.primaryCta}</Link>
            <a href={generalInquiryLink(dict)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center rounded border border-surface/60 px-4 py-2.5 text-xs font-medium text-surface transition-colors hover:bg-surface/10 sm:px-6 sm:py-3 sm:text-sm">{dict.finalCta.secondaryCta}</a>
          </div>
        </div>
        <div className="min-h-[260px] overflow-hidden bg-cover bg-center bg-no-repeat sm:min-h-[320px] lg:min-h-[360px]" role="img" aria-label="Sherinab Venture delivery truck" style={{ backgroundImage: `url(\"${imageUrl}\")` }} />
      </div>
    </section>
  );
}
