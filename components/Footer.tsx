...
export async function Footer({ locale, dict }: { locale: Locale; dict: Dictionary }) {
  ...
  return (
    <footer className="border-t border-border bg-white text-ink print:hidden">
      <div className="mx-auto max-w-content px-4 py-12 sm:px-6 sm:py-16">
        <Link href={base} className="inline-flex items-baseline font-display text-[2.15rem] font-medium leading-none tracking-tight text-ink transition-colors hover:text-brand sm:text-5xl">
          Sherinab <span className="ml-2 text-[0.72em] font-normal tracking-[0.18em] text-muted">VENTURE</span><span className="ml-1.5 text-brand">.</span>
        </Link>
        <p className="mt-5 max-w-2xl font-display text-lg leading-snug text-ink/80 sm:mt-6 sm:text-2xl">{mission}</p>
        <div className="mt-10 grid grid-cols-2 gap-x-8 gap-y-9 border-t border-border pt-8 sm:mt-14 sm:grid-cols-[1.3fr_0.85fr_0.65fr_1.2fr] sm:gap-8 sm:pt-12">
          <div>
            <h3 className="font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-muted">{dict.footer.productsHeading}</h3>
            ...
            <ul className="mt-4 space-y-1.5 font-display text-base text-ink/80 sm:mt-5 sm:space-y-2">
          ...
          <div>
            <h3 className="font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-muted">{dict.footer.businessHeading}</h3>
            <ul className="mt-4 space-y-2 font-display text-base text-ink/80">
          ...
          <div>
            <h3 className="font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-muted">{dict.footer.companyHeading}</h3>
            <ul className="mt-4 space-y-2 font-display text-base text-ink/80">
          ...
          <div className="col-span-2 border-t border-border pt-6 sm:col-span-1 sm:border-l sm:border-t-0 sm:border-border sm:pl-8 sm:pt-0">
            <h3 className="font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-muted">{locale === "fr" ? "ADRESSE" : "LOCATION"}</h3>
            <p className="mt-4 max-w-xs text-sm leading-6 text-muted [text-wrap:balance]">{address}</p>
          </div>
        </div>
        <div className="mt-9 flex flex-col gap-4 border-t border-border pt-6 sm:mt-16 sm:flex-row sm:items-center sm:justify-between sm:pt-7">
          ...
          <div className="inline-flex self-start rounded-full border border-border bg-white p-1 sm:self-auto"><LanguageSwitcher locale={locale} /></div>
        </div>
      </div>
    </footer>
  );
