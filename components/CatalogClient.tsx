        ...
        <div className="border-b border-border pb-4 sm:pb-5">
          ...
          <div className="mt-4 flex items-center gap-2">
            ...
            <div className="relative min-w-0 flex-1">
              ...
              <select id="category-filter" value={category ?? ""} onChange={(event) => setCategory(isCategorySlug(event.target.value) ? event.target.value : null)} className="h-10 w-full appearance-none rounded-md border border-border bg-background px-3 pr-9 text-sm font-medium text-ink outline-none transition-colors hover:border-brand/40 focus:border-brand focus:ring-2 focus:ring-brand/10">
            ...
            {hasFilters && <button type="button" onClick={clearAll} aria-label={dict.common.clearFilters} className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-md border border-border bg-background text-muted transition-colors hover:border-brand hover:text-brand"><X size={15} aria-hidden /></button>}
          </div>
        </div>
      </div>
